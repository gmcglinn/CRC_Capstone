<?php
    ...
    if(!isset($_POST['TID'])) {
        ...
        $sql = "SELECT * FROM s21_course_workflow_steps WHERE ATPID = '$TID'";
...
<?php
    ...
    if (isset($_POST['workflowCreate'])) {
		...
		if (isset($_POST['hiddeninput'])) {
			...
			$sql = "UPDATE s21_course_workflow_steps 
                    SET 
                        TSID = 1, 
                        workflow_title = '$workflow_title', 
                        instructions = '$newWorkflowOrder',
        	...
        	if (mysqli_errno($db_conn) == 0) {
            	echo("<div class='w3-panel w3-margin w3-green'><p>Course Successfully Modified.</p></div>");
        	} 
        	//Database detected duplicate entry
        	else if (mysqli_errno($db_conn) == 1062) {  
            echo("<div class='w3-panel w3-margin w3-red'><p>Failed to Modify Course.</p></div>");
        	}
        	else {
            	echo("<div class='w3-panel w3-margin w3-red'><p>Failed to Modify Course</p></div>");
			...
			echo("<div class='w3-panel w3-margin w3-red'><p>Failed to Modify Workflow. No Entry for workflow order</p></div>");
		}
...
<div id="workflowForm" class="w3-card-4 w3-padding w3-margin" style="display: block;">
    <h5 onload="LoadFormOptions()">Modify Existing Course (Workflow Template)</h5>
    <p>You can modify a custom course (workflow template) here.</p>
    <form id="subform" method="post">
        <label for="workflowTitle">Modified Course Title (Workflow Template Title)</label>
        ...
        <input type="submit" value="Modify Workflow Template" class="w3-button w3-teal w3-center" name="workflowCreate"></input>
